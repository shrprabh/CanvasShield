...
class CanvasFingerprintClassifier {
  ...
  extractFeatures(canvas, operations) {
    ...
    features.hasKnownTextPattern = operations.writes.some((op) =>
      this.features.textPatterns.some(
        (pattern) => op.text && op.text.includes(pattern)
      )
    );

    // Check canvas size
    features.hasSuspiciousSize =
      this.features.suspiciousSizes.includes(canvas.width) ||
        ...
        ...operations.reads.map((read) =>
          Math.min(
            ...operations.writes.map(
              (write) => read.timestamp - write.timestamp
            )
          )
        )
      );
      features.hasSmallTimeBetweenReadWrite =
        timeBetweenOps < this.features.readWriteTiming;
    }
  ...
  predict(canvas, operations) {
    ...
    if (features.hasSmallTimeBetweenReadWrite)
      score += this.weights.timingUnderThreshold;
    if (features.hasHighOperationCount)
      score += this.weights.operationCountOverThreshold;

    // Normalize score to [0,1]
    const maxPossibleScore = Object.values(this.weights).reduce(
      (a, b) => a + b,
      0
    );
...
window.__canvasWatcher = {
  init: function (callback) {
    ...
    // Override canvas methods to detect fingerprinting
